...
const mapDispatch = (dispatch) => ({
    ...
    updateChartData: (accountId, botId, startDate, endDate) => {
        dispatch(updateChartData(accountId, botId, startDate, endDate, metrics.metricType.TIME_ZONE));
    },
...
class TimeZoneAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentWillMount(nextProps) {
        ...
        setAnalyticsMenu(metrics.metricType.TIME_ZONE);
    ...
    componentWillUnmount() {
        ...
        removeMetric(accountId, botId, metrics.metricType.TIME_ZONE);
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data[metrics.metricType.TIME_ZONE]) {
            graphData = accountMetrics[this.props.botId].data[metrics.metricType.TIME_ZONE];
            tableData = accountMetrics[this.props.botId].data[metrics.metricType.TIME_ZONE_TABULAR_DATA];
            this.context.setTitle(title);
            return (
                <div>
                    <HelpLink content={languages.analytics.timeZone.helpContent} label="Time Zone" />

                    <div className={cx(s.root, flexbox.rowItem)} >
                        <PieChart metrics={graphData} label="Time Zone" />
                    </div>
                    {(tableData && tableData.data.columns.length) ?
                        <Table style={{ tableLayout: 'fixed' }} bodyStyle={{ overflow: 'visible' }}>
                            <TableHeader displaySelectAll={false} adjustForCheckbox={false}>
                                <TableRow>
                                    <TableHeaderColumn style={styles.tableCol}>Time Zone</TableHeaderColumn>
...
const TimeZoneAnalytics = connect(mapStateToProps, mapDispatch)(TimeZoneAnalyticsComponent);

export default withStyles(flexbox, s)(TimeZoneAnalytics);
