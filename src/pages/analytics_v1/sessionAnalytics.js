...
class SessionAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    componentWillMount(nextProps) {
        ...
        setAnalyticsMenu(metrics.metricType.MESSAGE_SESSION_ENGAGED);
    ...
    componentWillUnmount() {
        ...
        removeMetric(accountId, botId, metrics.metricType.MESSAGE_SESSION_ENGAGED);
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data.MessageSessionEngaged) {
            graphData = accountMetrics[this.props.botId].data.MessageSessionEngaged;
            graphSummaryData = accountMetrics[this.props.botId].data.MessageSessionEngagedSummary;
            ...
            return (
                <div>
                    <HelpLink content={languages.analytics.session.helpContent} label="Session Types" />
                    <SummaryMetric metricData={metricData} />
                    <div className={cx(s.root, flexbox.rowItem)} >
                        <LineChart metrics={graphData} label="Sessions" />
                    </div>
                </div>
            )
        } else {
...
const SessionAnalytics = connect(mapStateToProps, mapDispatch)(SessionAnalyticsComponent);

export default withStyles(flexbox, s)(SessionAnalytics);
