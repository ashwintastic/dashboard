...
const mapDispatch = (dispatch) => ({
    ...
    updateChartSummaryData: (accountId, botId, startDate, endDate, rangeFilter) => {
        if (rangeFilter != 'custom') {
            ...
            dispatch(updateChartData(accountId, botId, twiceStartDateString, endDateForSummary, metrics.metricType.ENGAGEMENT_TIME_SUMMARY));
        }
    },
...
class EngagementTimeAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentWillMount(nextProps) {
        ...
        setAnalyticsMenu(metrics.metricType.ENGAGEMENT_TIME);
    ...
    componentWillUnmount() {
        ...
        removeMetric(accountId, botId, metrics.metricType.ENGAGEMENT_TIME);
    }

    calculateAverage(data) {
        if (typeof data != 'undefined') {
            return Number(data.engagedTimeTotal / data.engagedTotal).toFixed(2);
        }
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data[metrics.metricType.ENGAGEMENT_TIME]) {
            graphData = accountMetrics[this.props.botId].data[metrics.metricType.ENGAGEMENT_TIME];
            graphSummaryData = accountMetrics[this.props.botId].data[metrics.metricType.ENGAGEMENT_TIME_SUMMARY];
            this.context.setTitle(title);

            let metricData = [];
            let metricObj = {}, pastSummaryAvg = this.calculateAverage(graphSummaryData);
            metricObj.metric = 'Avg. Engagement Time';
            ...
            return (
                <div>
                    <HelpLink content={languages.analytics.engagementTime.helpContent} label="Engagement Time" />
                    <SummaryMetric metricData={metricData} />
                    <div className={cx(s.root, flexbox.rowItem)} >
                        <PieChart metrics={graphData} label="Engagement Time" />
                    </div>
                </div>
            )
        } else {
...
const EngagementTimeAnalytics = connect(mapStateToProps, mapDispatch)(EngagementTimeAnalyticsComponent);

export default withStyles(flexbox, s)(EngagementTimeAnalytics);
