...
const mapDispatch = (dispatch) => ({
    updateChartData: (accountId, botId, startDate, endDate) => {
        dispatch(updateChartData(accountId, botId, startDate, endDate, metrics.metricType.TOP_TEXT_INPUTS));
    },
    removeFilter: () => {
...
class TopTextInputsAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentWillMount(nextProps) {
        ...
        setAnalyticsMenu(metrics.metricType.TOP_TEXT_INPUTS);
    ...
    componentWillUnmount() {
        ...
        removeFilter();
        removeMetric(accountId, botId, metrics.metricType.TOP_TEXT_INPUTS);
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data[metrics.metricType.TOP_TEXT_INPUTS]) {
            graphData = accountMetrics[this.props.botId].data[metrics.metricType.TOP_TEXT_INPUTS];
            this.context.setTitle(title);
            return (
                <div>
                    <HelpLink content={languages.analytics.topTextInputs.helpContent} label="Top Text Inputs" />
                    <h4 style={{ paddingTop: '1em' }}>Top Text Inputs</h4>
                    <Paper>
            ...
            /*<div className={cx(s.root, flexbox.rowItem)} style={{maxHeight:'600'}}>
                  <PieChart metrics={graphData} label="Top 50 Text Inputs" />
              </div>*/
        } else {
...
const TopTextInputsAnalytics = connect(mapStateToProps, mapDispatch)(TopTextInputsAnalyticsComponent);

export default withStyles(flexbox, s)(TopTextInputsAnalytics);
