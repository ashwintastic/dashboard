...
class MessageAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentWillMount(nextProps) {
        const { updateChartData, accountId, botId, startDate, endDate, rangeFilter,
            updateChartSummaryData, setAnalyticsMenu } = this.props;

        setAnalyticsMenu(metrics.metricType.MESSAGE_COUNT);
    ...
    componentWillUnmount() {
        ...
        removeMetric(accountId, botId, metrics.metricType.MESSAGE_COUNT);
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data.MessageCount) {
            graphData = accountMetrics[this.props.botId].data.MessageCount;
            graphSummaryData = accountMetrics[this.props.botId].data.MessageCountSummary;
            ...
            return (
                <div>
                    <HelpLink content={languages.analytics.messages.helpContent} label="Message Counts" />
                    <SummaryMetric metricData={metricData} />
                    <div className={cx(s.root, flexbox.rowItem)} >
                        <LineChart metrics={graphData} label="Messages" />
                    </div>
                </div>
            )
        } else {
...
const MessagesAnalytics = connect(mapStateToProps, mapDispatch)(MessageAnalyticsComponent);

export default withStyles(flexbox, s)(MessagesAnalytics);
