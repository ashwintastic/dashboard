...
const mapDispatch = (dispatch) => ({
    ...
    updateChartSummaryData: (accountId, botId, startDate, endDate, rangeFilter) => {
        if (rangeFilter != 'custom') {
            ...
            dispatch(updateChartData(accountId, botId, twiceStartDateString, endDateForSummary, metrics.metricType.DELIVERY_TIME_SUMMARY));
        }
    },
...
class DeliveryTimeAnalyticsComponent extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentWillMount(nextProps) {
        const { updateChartData, accountId, botId, startDate, endDate, updateChartSummaryData,
            rangeFilter, setAnalyticsMenu } = this.props;

        setAnalyticsMenu(metrics.metricType.DELIVERY_TIME);
    ...
    componentWillUnmount() {
        ...
        removeMetric(accountId, botId, metrics.metricType.DELIVERY_TIME);
    }

    calculateAverage(data) {
        if (typeof data != 'undefined') {
            return Number(data.deliveryTimeTotal / data.deliveredTotal).toFixed(2);
        }
    }
    ...
    render() {
        ...
        if (accountMetrics && accountMetrics[this.props.botId] && accountMetrics[this.props.botId].data[metrics.metricType.DELIVERY_TIME]) {
            graphData = accountMetrics[this.props.botId].data[metrics.metricType.DELIVERY_TIME];
            graphSummaryData = accountMetrics[this.props.botId].data[metrics.metricType.DELIVERY_TIME_SUMMARY];
            this.context.setTitle(title);

            let metricData = [];
            let metricObj = {}, pastSummaryAvg = this.calculateAverage(graphSummaryData);
            metricObj.metric = 'Avg. Delivery Time';
            ...
            return (
                <div>
                    <HelpLink content={languages.analytics.deliveryTime.helpContent} label="Delivery Time" />
                    <SummaryMetric metricData={metricData} />
                    <div className={cx(s.root, flexbox.rowItem)} >
                        <PieChart metrics={graphData} label="Delivery Time" />
                    </div>
                </div>
            )
        } else {
...
const DeliveryTimeAnalytics = connect(mapStateToProps, mapDispatch)(DeliveryTimeAnalyticsComponent);

export default withStyles(flexbox, s)(DeliveryTimeAnalytics);
